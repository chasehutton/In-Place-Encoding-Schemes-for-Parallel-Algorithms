...
void buffer_merge(parlay::sequence<uint32_t>& A, parlay::sequence<uint32_t>& B) {
	...
	for (size_t i = 0; i < g/2; i++) {
		...
		parlay::internal::merge_into<parlay::copy_assign_tag>(D, E, Buffer.aux, std::less<>());
		distribute(Buffer.aux, D, E);
		
		parlay::internal::merge_into<parlay::copy_assign_tag>(C, D, Buffer.aux, std::less<>());
		distribute(Buffer.aux, C, D);
		
		parlay::internal::merge_into<parlay::copy_assign_tag>(E, F, Buffer.aux, std::less<>());
	...
	for (size_t i = g/2; i < g; i++) {
		...
		parlay::internal::merge_into<parlay::copy_assign_tag>(D, E, Buffer1.aux, std::less<>());
		distribute(Buffer1.aux, D, E);
		
		parlay::internal::merge_into<parlay::copy_assign_tag>(C, D, Buffer1.aux, std::less<>());
		distribute(Buffer1.aux, C, D);
		
		parlay::internal::merge_into<parlay::copy_assign_tag>(E, F, Buffer1.aux, std::less<>());
